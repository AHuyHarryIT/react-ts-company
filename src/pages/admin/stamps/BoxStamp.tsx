...
export default function BoxStamp() {
  ...
  const formProps: FormProps<FormFields> = {
    ...
    form: form,
    onFinish: (values) => {
      // Auto calculate totalBox if hasComma
      const finalTotalBox = hasComma
        ? values.startBox.toString().split(',').length
        : values.totalBox;
            ...
            >
              <Input
                ...
                onChange={(e) => {
                  ...
                  // If hasComma changed, reset totalBox field
                  if (hasCommaInValue !== hasComma) {
                    form.setFieldValue('totalBox', undefined);
                  }
                }}
              />
            </Form.Item>
